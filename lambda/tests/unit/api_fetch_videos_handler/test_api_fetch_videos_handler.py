...
# Lambda handler path
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../api_fetch_videos_handler")))
...
class TestLambdaHandler:
    ...
    def test_lambda_handler_success(self, mock_add_presigned_urls, mock_vector_db_service):
        # Setup mocks
    ...
    @patch('lambda_function.add_presigned_urls')
    @patch('lambda_function.get_secret')
    def test_lambda_handler_with_environment_variables(self, mock_get_secret, mock_add_presigned_urls):
        # Mock the SECRET_NAME at module level
        with patch('lambda_function.SECRET_NAME', 'custom_secret'), \
             patch('lambda_function.PRESIGNED_URL_EXPIRY', 7200), \
             patch('lambda_function.VectorDBService') as mock_vector_db_service:
            
            mock_vector_db = MagicMock()
            mock_vector_db.fetch_videos.return_value = ([], 0)
            mock_vector_db_service.return_value = mock_vector_db
            mock_get_secret.return_value = {"DB_USERNAME": "test", "DB_PASSWORD": "test"}
