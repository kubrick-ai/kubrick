...
sys.path.insert(
    ...
    os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../../../src/sqs_embedding_task_producer"
        )
    ),
)
...
class TestLambdaHandler:
    ...
    def test_lambda_handler_success_mp4_file(
        ...
        assert wait_for_file_call_args[0][:4] == (
            "test-bucket",
        ...
        mock_presigned_url.assert_called_once_with(
            bucket="test-bucket", key="videos/test_video.mp4", expires_in=600
        )
    ...
    def test_lambda_handler_success_various_video_formats(
        ...
        for video_file in video_formats:
            ...
            with patch.dict(
                "os.environ",
                {"AWS_DEFAULT_REGION": "us-east-1", "QUEUE_URL": queue_url},
            ):
                response = lambda_handler(event, context)
    ...
    @patch("lambda_function.EmbedService")
    @patch("lambda_function.VectorDBService")
    def test_lambda_handler_invalid_file_type(
        ...
        for invalid_file in invalid_files:
            ...
            assert (
                "not a video or the video format is not supported"
                in response["message"]
            )
    ...
    @patch("lambda_function.EmbedService")
    @patch("lambda_function.VectorDBService")
    def test_lambda_handler_folder_creation_event(
        self, mock_vector_db_service, mock_embed_service
    ):
    ...
    def test_lambda_handler_embed_service_failure(
        ...
        # Mock services
        mock_embed_service_instance = MagicMock()
        mock_embed_service_instance.create_embedding_request.side_effect = Exception(
    ...
    def test_lambda_handler_database_store_task_failure(
        ...
        mock_vector_db_instance = MagicMock()
        mock_vector_db_instance.store_task.side_effect = Exception(
    ...
    @patch("lambda_function.EmbedService")
    @patch("lambda_function.VectorDBService")
    def test_lambda_handler_missing_s3_records(
        self, mock_vector_db_service, mock_embed_service
    ):
    ...
    @patch("lambda_function.EmbedService")
    @patch("lambda_function.VectorDBService")
    def test_lambda_handler_malformed_s3_record(
        self, mock_vector_db_service, mock_embed_service
    ):
