...
class TestHasValidFileExtension(TestVideoValidator):
    def test_valid_extensions(self):
        ...
        for file_path in valid_files:
            assert video_validation.has_valid_file_extension(file_path)
    ...
    def test_invalid_extensions(self):
        ...
        for file_path in invalid_files:
            assert not video_validation.has_valid_file_extension(file_path)

    def test_case_insensitive_extensions(self):
        case_variations = ["video.MP4", "movie.AVI", "clip.MOV", "film.MKV"]
...
class TestHasVideoStream(TestVideoValidator):
    def test_valid_video_stream(self):
        result = video_validation.has_video_stream(self.valid_probe_data)
        assert result is True

    def test_no_video_stream(self):
        result = video_validation.has_video_stream(self.invalid_probe_data_no_video)
    ...
    def test_only_image_streams(self):
        ...
        result = video_validation.has_video_stream(image_probe_data)
        assert result is False
    ...
    def test_mixed_streams(self):
        ...
        result = video_validation.has_video_stream(mixed_probe_data)
        assert result is True


class TestIsValidDuration(TestVideoValidator):
    def test_valid_duration(self):
        result = video_validation.is_valid_duration(self.valid_probe_data)
        assert result is True

    def test_minimum_duration(self):
        probe_data = {"format": {"duration": str(video_validation.MIN_DURATION)}}

        result = video_validation.is_valid_duration(probe_data)
        assert result is True

    def test_maximum_duration(self):
        probe_data = {"format": {"duration": str(video_validation.MAX_DURATION)}}

        result = video_validation.is_valid_duration(probe_data)
        assert result is True

    def test_duration_too_short(self):
        probe_data = {"format": {"duration": str(video_validation.MIN_DURATION - 1)}}

        result = video_validation.is_valid_duration(probe_data)
        assert result is False

    def test_duration_too_long(self):
        probe_data = {"format": {"duration": str(video_validation.MAX_DURATION + 1)}}
    ...
    def test_invalid_duration_format(self):
        probe_data = {"format": {"duration": "invalid"}}
...
class TestIsValidDimensions(TestVideoValidator):
    def test_valid_dimensions(self):
        result = video_validation.is_valid_dimensions(self.valid_probe_data)
        assert result is True
    ...
    def test_minimum_dimensions(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MIN_WIDTH,
                    "height": video_validation.MIN_HEIGHT,
                }
            ]
        }

        result = video_validation.is_valid_dimensions(probe_data)
        assert result is True
    ...
    def test_maximum_dimensions(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MAX_WIDTH,
                    "height": video_validation.MAX_HEIGHT,
                }
            ]
        }
    ...
    def test_width_too_small(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MIN_WIDTH - 1,
                    "height": video_validation.MIN_HEIGHT,
                }
            ]
        }
    ...
    def test_width_too_large(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MAX_WIDTH + 1,
                    "height": video_validation.MIN_HEIGHT,
                }
            ]
        }
    ...
    def test_height_too_small(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MIN_WIDTH,
                    "height": video_validation.MIN_HEIGHT - 1,
                }
            ]
        }
    ...
    def test_height_too_large(self):
        probe_data = {
            "streams": [
                {
                    "width": video_validation.MIN_WIDTH,
                    "height": video_validation.MAX_HEIGHT + 1,
                }
            ]
        }
...
class TestIsValidFilesize(TestVideoValidator):
    ...
    def test_valid_filesize_small_file(self, mock_getsize):
        ...
        result = video_validation.is_valid_filesize(self.valid_file_path)
    ...
    def test_filesize_with_nonexistent_file(self):
        with pytest.raises(OSError):
            video_validation.is_valid_filesize("nonexistent_file.mp4")
...
class TestIsValidVideoFile(TestVideoValidator):
    ...
    def test_file_does_not_exist(self, mock_exists):
        ...
        result = video_validation.is_valid_video_file(self.invalid_file_path)
    ...
    def test_invalid_file_extension(self, mock_extension, mock_exists):
        ...
        result = video_validation.is_valid_video_file("test.txt")
    ...
    def test_probe_data_none(self, mock_probe, mock_extension, mock_exists):
        ...
        result = video_validation.is_valid_video_file(self.valid_file_path)
...
class TestConstants(TestVideoValidator):
    ...
    def test_valid_extensions_list(self):
        ...
        for ext in expected_extensions:
            assert ext in video_validation.VALID_VIDEO_EXTENSIONS

    def test_dimension_constraints(self):
        assert video_validation.MAX_WIDTH > video_validation.MIN_WIDTH
        assert video_validation.MAX_HEIGHT > video_validation.MIN_HEIGHT
        assert video_validation.MIN_WIDTH > 0
        assert video_validation.MIN_HEIGHT > 0

    def test_duration_constraints(self):
        assert video_validation.MAX_DURATION > video_validation.MIN_DURATION
        assert video_validation.MIN_DURATION > 0

    def test_filesize_constraint(self):
        assert video_validation.MAX_FILESIZE > 0
        assert video_validation.MAX_FILESIZE == 2000000000  # 2GB
