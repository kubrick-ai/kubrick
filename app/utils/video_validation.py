...
def is_valid_video_file(file_path: str) -> bool:
    ...
    return (
        is_valid_filesize(file_path)
        and has_video_stream(probe_data)
        and is_valid_duration(probe_data)
        and is_valid_dimensions(probe_data)
    )
...
def has_valid_file_extension(file_path: str) -> bool:
    ...
    if file_ext in VALID_VIDEO_EXTENSIONS:
        ...
        print("Invalid file extension. Check if file type supported")
        return False
...
def has_video_stream(probe_data: Dict[str, Any]) -> bool:
    ...
    if not video_streams:
        print(
            "No video stream detected. Check if file is corrupted or file type supported"
        )
        return False
    ...
    if len(legitimate_video_streams) > 0:
        ...
        print("No valid video stream detected. Check if file type supported")
        return False
...
def is_valid_duration(probe_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if MIN_DURATION <= duration_float and duration_float <= MAX_DURATION:
            ...
            print(
                f"Invalid video duration. Video must duration be between {MIN_DURATION} and {MAX_DURATION} seconds"
            )
            return False
...
def is_valid_dimensions(probe_data: Dict[str, Any]) -> bool:
    ...
    if width < MIN_WIDTH or width > MAX_WIDTH:
        print(
            f"Invalid video width. Video must be between {MIN_WIDTH} and {MAX_WIDTH} pixels wide"
        )
        return False
    if height < MIN_HEIGHT or height > MAX_HEIGHT:
        print(
            f"Invalid video height. Video must be between {MIN_HEIGHT} and {MAX_HEIGHT} pixels in height"
        )
        return False
    return True
